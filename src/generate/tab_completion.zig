...
// Hard cap for comptime node collection. Keeping this bounded gives a clear
// compile-time failure mode for exceptionally large command trees.
const fish_tab_completion_max_nodes: usize = 4096;
// Single quota for fish generation. Traversal + emission are fully comptime,
// so this must be high enough for large trees.
const fish_tab_completion_branch_quota: u32 = 10_000;
// Optional diagnostics in generated fish output. When enabled, a stats comment
// line is emitted at the top of fish completions for empirical tuning.
const fish_tab_completion_log_stats: bool = false;

const FishTabCompletionStats = struct {
    nodes: usize = 0,
    opts: usize = 0,
    max_depth: usize = 0,
    desc_bytes: usize = 0,
...
fn FishTabCompletionNode(comptime CommandT: type) type {
    return struct {
        cmd: CommandT,
...
fn fishTabCompletionNextCtx(comptime tc_ctx: TabCompletionContext, comptime sub_name: []const u8) TabCompletionContext {
    // Keep context updates in one place so traversal and emitter stay simple.
    var next_ctx = tc_ctx;
    next_ctx.parent_name = (if (tc_ctx.parent_name.len == 0) "" else tc_ctx.parent_name ++ "_") ++ tc_ctx.name;
    next_ctx.idx += 1;
    next_ctx.name = sub_name;
    return next_ctx;
}

fn fishTabCompletionCollect(
    comptime CommandT: type,
    comptime root_cmd: CommandT,
    comptime root_ctx: TabCompletionContext,
) struct {
    nodes: [fish_tab_completion_max_nodes]FishTabCompletionNode(CommandT),
    len: usize,
    stats: FishTabCompletionStats,
} {
    comptime {
        // Collect once with an explicit LIFO stack (iterative DFS) to avoid
        // recursive comptime calls that scale poorly with deep command trees.
        const Node = FishTabCompletionNode(CommandT);

        var out_buf: [fish_tab_completion_max_nodes]Node = undefined;
        var out_len: usize = 0;

        var stack: [fish_tab_completion_max_nodes]Node = undefined;
        var sp: usize = 0;

        var stats: FishTabCompletionStats = .{
            .max_depth = root_ctx.idx,
        };

        stack[0] = .{ .cmd = root_cmd, .ctx = root_ctx };
        sp = 1;
        ...
        while (sp > 0) {
            ...
            if (out_len >= fish_tab_completion_max_nodes)
                @compileError("Fish tab completion command tree exceeded fish_tab_completion_max_nodes.");

            out_buf[out_len] = cur;
            out_len += 1;

            stats.nodes += 1;
            if (cur.ctx.idx > stats.max_depth) stats.max_depth = cur.ctx.idx;

            if (cur.ctx.include_cmds) {
                if (cur.cmd.sub_cmds) |sub_cmds| {
                    for (sub_cmds) |sub_cmd| {
                        stats.desc_bytes += sub_cmd.description.len;
                    }

                    var i: usize = sub_cmds.len;
                    while (i > 0) {
                        i -= 1;
                        const sub_cmd = sub_cmds[i];
                        const next_ctx = fishTabCompletionNextCtx(cur.ctx, sub_cmd.name);

                        // Push in reverse so pop order matches declared order.
                        // This preserves deterministic script output.

                        if (sp >= fish_tab_completion_max_nodes)
                            @compileError("Fish tab completion traversal stack exceeded fish_tab_completion_max_nodes.");

                        stack[sp] = .{ .cmd = sub_cmd, .ctx = next_ctx };
                        sp += 1;
                    }
                }
            }

            if (cur.ctx.include_opts) {
                if (cur.cmd.opts) |opts| {
                    stats.opts += opts.len;
                    for (opts) |opt| {
                        stats.desc_bytes += opt.description.len;
                    }
                }
            }
        }
...
fn cmdTabCompletionFish(
    ...
    // Set once for the whole fish generation pass (collection + emission).
    @setEvalBranchQuota(fish_tab_completion_branch_quota);

    // Flat list of nodes plus aggregate stats used for diagnostics/tuning.
    const collected = comptime fishTabCompletionCollect(CommandT, cmd, tc_ctx);
    const nodes = collected.nodes[0..collected.len];

    if (comptime fish_tab_completion_log_stats) {
        // Emitted as a comment to keep generated scripts valid and readable.
        try tc_writer.print(
            "# fish tab completion stats: root={s} nodes={d} opts={d} max-depth={d} desc-bytes={d} quota={d}\n",
            .{
                tc_ctx.root_name,
                collected.stats.nodes,
                collected.stats.opts,
                collected.stats.max_depth,
                collected.stats.desc_bytes,
                fish_tab_completion_branch_quota,
            },
        );
    }

    inline for (nodes) |node| {
        try cmdTabCompletionFishOne(CommandT, node.cmd, tc_writer, node.ctx);
    }
}
